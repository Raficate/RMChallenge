...
@Component({
  selector: 'app-list-desafios',
  templateUrl: './list-desafios.page.html',
  styleUrls: ['./list-desafios.page.scss'],
})
export class ListDesafiosPage implements OnInit {
  ...
  async cargarEjercicioAleatorio() {
    try {
      this.ejercicios = await firstValueFrom(this.firestoreService.getCollectionDesafios());
  ...
  async marcarPendiente() {
    ...
    try {
      await this.firestoreService.updateDocument('desafios', this.ejercicio.id!, {
