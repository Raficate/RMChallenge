...
@Component({
  selector: 'app-list-pesos',
  templateUrl: './list-pesos.page.html',
  styleUrls: ['./list-pesos.page.scss'],
})
export class ListPesosPage implements OnInit {
  ...
  async cargarEjercicioAleatorio() {
    try {
      this.ejercicios = await firstValueFrom(this.firestoreService.getCollectionPesos());
      ...
      if (ejerciciosNoHechos && ejerciciosNoHechos.length > 0) {
        ...
        this.router.navigate(['/sin-ejercicios/pesos']);
      }
    } catch (error) {
  ...
  async marcarPendiente() {
    ...
    try {
      await this.firestoreService.updateDocument('pesos', this.ejercicio.id!, {
