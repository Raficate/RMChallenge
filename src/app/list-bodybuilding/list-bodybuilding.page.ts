...
@Component({
  selector: 'app-list-bodybuilding',
  templateUrl: './list-bodybuilding.page.html',
  styleUrls: ['./list-bodybuilding.page.scss'],
})
export class ListBodybuildingPage implements OnInit {
  ...
  async cargarEjercicioAleatorio() {
    try {
      this.ejercicios = await firstValueFrom(this.firestoreService.getCollectionBodyBuilding());
  ...
  async marcarPendiente() {
    // Cambiar la variable 'hecho' a 'P'
    this.ejercicio.hecho = 'P';

    try {
      await this.firestoreService.updateDocument('bodybuilding', this.ejercicio.id!, {
